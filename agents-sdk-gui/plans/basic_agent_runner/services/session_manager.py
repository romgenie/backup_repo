...
class SessionManager:
    ...
    def _sanitize_user_input(content: str) -> str:
        ...
        # Escape angle brackets to prevent HTML/XML injections and tool syntax abuse
        sanitized = sanitized.replace("<", "&lt;").replace(">", "&gt;")
        
        # Replace any attempt to use tool syntax directly
        patterns_to_replace = [
            (r"&lt;tool&gt;", "[tool]"),
            (r"&lt;/tool&gt;", "[/tool]"),
            (r"&lt;args&gt;", "[args]"),
        ...
        # Prevent impersonation of system messages
        if sanitized.strip().startswith("system:") or sanitized.strip().startswith("System:"):
            sanitized = "User input: " + sanitized
    ...
    def add_user_message(content: str) -> None:
        ...
        sanitized_content = SessionManager._sanitize_user_input(content)
        
        st.session_state.conversation_history.append({
            "role": "user",
            "content": sanitized_content
        })
        print(f"Added user message: {sanitized_content[:50]}...")
