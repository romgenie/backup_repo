...
def on_agent_create(app, agent, run_config):
    ...
    # Store in session state using SessionManager AND directly in st.session_state
    # First store using SessionManager 
    SessionManager.set_agent(agent, run_config)
    
    # Also store directly to ensure it's available immediately
    st.session_state.agent = agent
    st.session_state.run_config = run_config
    
    # Set the agent_creating flag to indicate successful creation
    SessionManager.set_flag("agent_creating", True)
...
def handle_mcp_server_add(app, server_id: Optional[str], server_config: Dict[str, Any]):
...
def handle_mcp_server_remove(app, server_id: str):
...
def handle_mcp_server_test(app, server_config: Dict[str, Any]):
