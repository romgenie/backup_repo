...
class ToolConverter:
    ...
    with col2:
        ...
        store = st.checkbox("Store response", True)
    ...
    parallel_tool_calls = st.checkbox("Enable parallel tool calls") if use_tools else False
    ...
    if st.button("Generate Preview"):
        ...
        settings_args = []
        if temperature != 0.7:
            settings_args.append(f"temperature={temperature}")
        if top_p != 1.0:
            settings_args.append(f"top_p={top_p}")
        if frequency_penalty != 0.0:
            settings_args.append(f"frequency_penalty={frequency_penalty}")
        if presence_penalty != 0.0:
            settings_args.append(f"presence_penalty={presence_penalty}")
        if max_tokens != 1000:
            settings_args.append(f"max_tokens={max_tokens}")
        if use_tools and tool_choice != "auto":
            settings_args.append(f"tool_choice='{tool_choice}'")
        if use_tools and parallel_tool_calls:
            settings_args.append(f"parallel_tool_calls={parallel_tool_calls}")
        if not store:
            settings_args.append(f"store={store}")
        
        settings_str = "ModelSettings()" if not settings_args else f"ModelSettings(\n    {',\n    '.join(settings_args)}\n)"
        
        # Generate sample code - with corrected class name
        sample_code = f"""
import asyncio
from agents.models.openai_provider import OpenAIProvider
from agents.model_settings import ModelSettings
...
async def main():
    # Initialize provider and get model
...
asyncio.run(main())
        ...
        # Show expected OpenAI API call
        st.subheader("Equivalent OpenAI API Call")
        ...
        if temperature != 0.7:
            openai_call["temperature"] = temperature
        if top_p != 1.0:
            openai_call["top_p"] = top_p
        if frequency_penalty != 0.0:
            openai_call["frequency_penalty"] = frequency_penalty
        if presence_penalty != 0.0:
            openai_call["presence_penalty"] = presence_penalty
        if max_tokens != 1000:
            openai_call["max_tokens"] = max_tokens
        if use_tools and tool_choice != "auto":
            openai_call["tool_choice"] = tool_choice
        if use_tools and parallel_tool_calls:
            openai_call["parallel_tool_calls"] = parallel_tool_calls
        if not store:
            openai_call["store"] = store
        
        st.code(json.dumps(openai_call, indent=2), language="json")