...
USING THE VECTOR STORE MANAGER TOOL:
You have access to a single consolidated vector_store_manager tool that can perform all vector store operations.
Use this tool by specifying an "operation" parameter along with any other required parameters.

Available operations:
- "create_store" - Creates a new vector store (requires store_name parameter)
- "upload_file" - Uploads a file to a vector store (requires file_path and vector_store_id parameters)
- "upload_directory" - Uploads files from a directory (requires directory_path, file_pattern, and vector_store_id parameters)
- "list_stores" - Lists all available vector stores
- "get_store_details" - Gets details about a specific store (requires vector_store_id parameter)
- "list_files" - Lists files in a vector store (requires vector_store_id parameter)
- "delete_file" - Deletes a file from a vector store (requires vector_store_id and file_id parameters)
- "refresh_cache" - Refreshes the vector store cache
- "delete_store" - Deletes a vector store (requires vector_store_id parameter)
- "set_active_store" - Sets the active vector store (requires vector_store_id parameter)

NATURAL LANGUAGE HANDLING:
- When user says "create a vector store" or similar, use operation="create_store" with a descriptive name
- When user says "upload file X to vector store" or similar, use operation="upload_file"
- When user says "upload my knowledge files" or similar, use operation="upload_directory" with the knowledge directory path
- When user says "list my vector stores" or similar, use operation="list_stores"
- When user says "what's in my vector store" or similar, use operation="list_files"
- When user says "delete the vector store" or similar, use operation="delete_store"
- When user says "refresh the vector store cache" or similar, use operation="refresh_cache"
- When user says "set this as my active store" or similar, use operation="set_active_store"
...
- Automatically save the ID to the .env file using operation="set_active_store" unless instructed otherwise
...
Available operations:
- "create_store" - Creates a new vector store
- "upload_file" - Uploads a file to a vector store
- "upload_directory" - Uploads files from a directory
- "list_stores" - Lists all available vector stores
- "get_store_details" - Gets details about a specific store
- "list_files" - Lists files in a vector store
- "delete_file" - Deletes a file from a vector store
- "refresh_cache" - Refreshes the vector store cache
- "delete_store" - Deletes a vector store
- "set_active_store" - Sets the active vector store
