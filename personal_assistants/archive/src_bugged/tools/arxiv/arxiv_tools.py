"""
ArXiv search and download tools using the arxiv Python package.

This module provides comprehensive functionality for searching and downloading
academic papers from ArXiv. It includes features for detailed searches by query,
author, category, and other parameters, as well as single and batch downloading
capabilities with proper error handling and retry mechanisms.

Key Features:
- Advanced search with multiple filtering options (categories, authors, date ranges)
- Paper downloads with proper error handling and retry logic
- Batch downloading of multiple papers
- Combined search-and-download operations
- Smart rate limiting to avoid API throttling
- Comprehensive error handling and reporting
- Integration with knowledge management workflows

Example:
    # Search for papers with category filtering
    ...
    download_result = download_paper_from_arxiv(
        paper_id="1706.03762",
        download_dir="/path/to/save"
    )
    
    # Search and download in one operation
    combined_result = search_and_download_papers(
...
Notes:
    This module is designed to be used with the OpenAI Agents SDK for seamless
    integration with LLM-based agents and workflows. When used as function tools,
    these functions provide a robust way for agents to interact with the ArXiv
...
def search_arxiv(
    ...
    """
    # Defaults are now handled in the function signature
    try:
        # Validate inputs
...
def download_paper_from_arxiv(
    ...
    Args:
        paper_id: ArXiv paper ID (e.g., "1706.03762" or "arxiv:1706.03762" or "2407.15516v1")
...
def search_and_download_papers(
    ...
    # Download the papers
    download_result = batch_download_papers(
        paper_ids=paper_ids,
        download_dir=download_dir,
        overwrite=overwrite
    )
